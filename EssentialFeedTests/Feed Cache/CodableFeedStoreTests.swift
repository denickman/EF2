...
final class CodableFeedStoreTests: XCTestCase {
    ...
    func test_insert_overridesPreviouslyInsertedCacheValues() {
        ...
        let firstInsertionError = insert((uniqueImagesFeed().local, Date()), to: sut)
...
//    }
    ...
    func test_delete_emptiesPreviouslyInsertedCache() {
        // delete the previously inserted cache
    ...
    private func insert(_ cache: (feed: [LocalFeedImage], timestamp: Date), to sut: FeedStore) -> Error? {
        let expectation = expectation(description: "Wait for the completion")
        var insertionError: Error?
        
        sut.insert(cache.feed, timestamp: cache.timestamp) { receivedError in
            // must get no error here
